...
def clear_neo4j_database():
    ...
    print("✅ Cleared all nodes and relationships from Neo4j database")

def create_knowledge_graph(nodes: List[Dict], relationships: List[Dict]):
    """Create proper knowledge graph structure in Neo4j."""
    with driver.session() as session:
        # Create nodes
        for node in nodes:
            node_type = node["type"]
            ...
            # Create node with proper labels
            session.run(f"""
                CREATE (n:{node_type} {{
        ...
        for rel in relationships:
            ...
            # Create relationship with proper matching
            session.run(f"""
                MATCH (a), (b)
                WHERE (a.id = $from_node OR a.course_id = $from_node)
                AND b.id = $to_node
                CREATE (a)-[r:{rel_type} {{
                    {', '.join([f'{k}: ${k}' for k in properties.keys()])}
                }}]->(b)
    ...
    print(f"✅ Created knowledge graph with {len(nodes)} nodes and {len(relationships)} relationships")
